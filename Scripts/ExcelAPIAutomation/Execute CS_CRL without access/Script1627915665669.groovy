...
'Verifying if excel auth token  for benake API response status code is 201'
WS.verifyResponseStatusCode(Response, 201)

'Sending request to Execute CS_CRL without access API and capturing response in ExecuteRecWithoutAccessResponse variable'
ExecuteCS_CSviewWithoutAccessResponse = WS.sendRequest(findTestObject('ExcelAPIAutomation/Execute CS_CRL without access', [('benake_auth_token') : GlobalVariable.benake_auth_token]))

'Printing Execute CS_CRL without access API Response'
...
'Parsing Execute CS_CRL without access API Response'
def result = sluper.parseText(ExecuteCS_CSviewWithoutAccessResponse.getResponseBodyContent())

'Parsing Execute CS_CRL without access API Response and extracting value of status key and verifying whether it error'
WS.verifyElementPropertyValue(ExecuteCS_CSviewWithoutAccessResponse, '[0].data_items[0][0].status', "error")

'Parsing Execute CS_CRL API Response and extracting value of message key and verifying whether user is You don\'t have access to the requested data.'
...
'Parsing Execute CS_CRL without access API Response and extracting value of cid key and verifying whether it r1'
WS.verifyElementPropertyValue(ExecuteCS_CSviewWithoutAccessResponse, '[0].cid', "cr_1")

'Parsing Execute CS_CRL without access API Response and extracting value of status key and verifying whether it data'
