...
'Sending request to get excel users API and capturing response in getexcelusersResponse variable'
getexcelusersResponse = WS.sendRequest(findTestObject('ExcelAPIAutomation/Get excel Users'))

'Printing Get excel users Response API response'
println(getexcelusersResponse.getResponseText())

'Verifying if get excel users Response API response is 200'
WS.verifyResponseStatusCode(getexcelusersResponse, 200)
...
println GlobalVariable.newuser
WS.delay(2)
